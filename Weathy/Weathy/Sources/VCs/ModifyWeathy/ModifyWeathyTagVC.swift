...
//                print(self.myClothesTagData)
    ...
    func callModifyWeathyService() {
        ModifyWeathyService.shared.modifyWeathy(userId: UserDefaults.standard.integer(forKey: "userId"), token: UserDefaults.standard.string(forKey: "token")!, date: dateString, code: locationCode, clothArray: selectedTags, stampId: weathyData?.stampId ?? -1, feedback: weathyData?.feedback ?? "", img: selectedImage, isDelete: false, weathyId: weathyData?.weathyId ?? -1) { (networkResult) -> (Void) in
